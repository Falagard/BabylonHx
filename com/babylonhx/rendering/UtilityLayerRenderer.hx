...
@:expose('BABYLON.OutlineRenderer') class UtilityLayerRenderer {
	...
	public function new(originalScene:Scene, handleEvents:Bool = true) {
		this.meshesSelectionPredicate = function(m:AbstractMesh):Bool {
			return originalScene.activeCamera != null
				&& m.material != null
				&& m.isVisible
				&& m.isEnabled()
				&& m.isBlocker
				&& ((m.layerMask & originalScene.activeCamera.layerMask) != 0);
		}
			...
			this._originalPointerObserver = originalScene.onPrePointerObservable.add(function(prePointerInfo, _) {
				...
				var getNearPickDataForScene = function(scene:Scene) {
					...
					if (prePointerInfo.nearInteractionPickingInfo != null) {
						if (prePointerInfo.nearInteractionPickingInfo.pickedMesh != null
							&& prePointerInfo.nearInteractionPickingInfo.pickedMesh.getScene() == scene) {
						...
						scenePick = prePointerInfo.ray != null ? scene.pickWithRay(prePointerInfo.ray,
							null) : scene.pick(originalScene.pointerX, originalScene.pointerY);
				...
				if (this.onlyCheckPointerDownEvents && prePointerInfo.type != PointerEventTypes.POINTERDOWN) {
					if (!prePointerInfo.skipOnPointerObservable) {
						this.utilityLayerScene.onPointerObservable.notifyObservers(new PointerInfo(prePointerInfo.type, prePointerInfo.event,
							utilityScenePick), prePointerInfo.type);
					}
				...
				if (this.utilityLayerScene.autoClearDepthAndStencil || this.pickUtilitySceneFirst) {
					...
					if (originalScenePick != null && utilityScenePick != null) {
						...
						if (utilityScenePick.distance == 0 && originalScenePick.pickedMesh != null) {
						...
						} else if (!this._pointerCaptures[pointerEvent.pointerId]
							&& (utilityScenePick.distance < originalScenePick.distance || originalScenePick.distance == 0)) {
							...
							&& utilityScenePick.distance >= originalScenePick.distance) {
